...
- ruff format + check
- mypy
- module-size check (500 lines max per source file)
- pytest (unit + integration)
- conventional-pre-commit (commit message validation -- strict, scoped)
- detect-secrets
- file validation (YAML, JSON, TOML)
- import-linter (architectural boundary enforcement) -- disabled until an `.importlinter` contract is added
...
- **Scopes**: `your_package,deps,repo` (set per project — see [docs/use-this-template.md](docs/use-this-template.md))
