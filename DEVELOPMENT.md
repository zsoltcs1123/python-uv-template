...
```bash
make check              # Run all checks (format, lint, lint-md, type-check, test)
...
make lint-md      # Lint markdown files
make lint-md-fix  # Lint and auto-fix markdown files
...
**pymarkdownlnt** ensures consistent markdown formatting.
...
uv run pymarkdown scan README.md  # Specific file

# List all available rules
uv run pymarkdown plugins list

# Get info about a specific rule
uv run pymarkdown plugins info md013
```

**Configuration**: `.markdownlint.json` (shared by both pymarkdownlnt and VS Code extension)

**Common configuration options:**

- Line length: `MD013` (default: 80, recommended: 120)
- Inline HTML: `MD033` (disable if you use HTML in markdown)
- First line heading: `MD041` (disable for flexibility)
- Code block style: `MD046` (use "fenced" for consistency)

**Documentation**: [PyMarkdown Docs](https://pymarkdown.readthedocs.io/)

**VS Code Extension**: Install [markdownlint](https://marketplace.visualstudio.com/items?itemName=DavidAnson.vscode-markdownlint) - automatically uses `.markdownlint.json`
...
- `.markdownlint.json` - Markdown linting rules (shared by pymarkdownlnt and VS Code extension)
